                ...
                if (listOfBooks[i].isbn == t) {
                    for (int j = 0; j < listOfUsers.size(); j++) {
                        if (listOfUsers[j].id == listOfBooks[i].issuedto) {
                            if (listOfUsers[j].usertype == 1)
                                cout << "Book is issued by: " << listOfUsers[j].id << " and due date is "
                                     << listOfBooks[i].showDueDate(1) << "\n";
                            else
                                cout << "Book is issued by: " << listOfUsers[j].id << " and the due date is "
                                     << listOfBooks[i].showDueDate(2) << "\n";
                        }
                    }
                }
            }
                ...
                if (i.id == temp) {
                    ...
                    for (auto j: i.listofissuedbooks) {
                        for (auto p: listOfBooks) {
                            if (p.isbn == j) {
                                cout << "   Name: " << p.title << " | ISBN: " << p.isbn << " | Due Date: "
                                     << p.showDueDate(1) << ".\n";
                            }
                        }
                    }
        ...
        if (c == '1') {
            ...
            for (auto i: this->listofissuedbooks) {
                ...
                for (auto j: listOfBooks) {
                    if (i == j.isbn) {
                        cout << "   " << "Name: " << j.title << " | Author: " << j.author << " | ISBN: " << j.isbn
                             << " | Publication: " << j.publication << " | Due Date: " << j.showDueDate(2) << "\n";
                    }
                }
            }
        } else if (c == '3') {
            BookDatabase temp;
            temp.displayAvailableBooks();
        } else if (c == '4') {
            BookDatabase temp;
        ...
        if (c == '1') {
            ...
            for (auto i: this->listofissuedbooks) {
                for (auto j: listOfBooks) {
                    if (i == j.isbn) {
                        cout << "   " << "Name: " << j.title << " | Author: " << j.author << " | ISBN: " << j.isbn
                             << " | Publication: " << j.publication << " | Due Date: " << j.showDueDate(1) << "\n";
                    }
                }
            }
        } else if (c == '3') {
...
void BookDatabase::displayBooks() {
    for (auto i: listOfBooks) {
        cout << "   " << "Name: " << i.title << " | Author: " << i.author << " | ISBN: " << i.isbn << " | Publication: "
             << i.publication << "\n";
    }
}
...
void BookDatabase::checkAvailability() {
    ...
    while (t == "")
        getline(cin, t);// Loop because getline works differently and takes some extra space from previous input.
    int flag = 0, done = 0;
    for (auto i: listOfBooks) {
        if (i.title == t) {
            flag = 1;
        ...
        if (((*itr).isbn) == t) {
            ...
            for (int k = 0; k < listOfStudents.size(); k++) {
                if (listOfStudents[k].id == uid) {
                    ...
                    for (it = listOfStudents[k].listofissuedbooks.begin();
                         it != listOfStudents[k].listofissuedbooks.end(); it++) {
                        if (*it == t) {
            ...
            for (int k = 0; k < listOfProfessors.size(); k++) {
                if (listOfProfessors[k].id == uid) {
                    vector<string>::iterator it;
                    for (it = listOfProfessors[k].listofissuedbooks.begin();
                         it != listOfProfessors[k].listofissuedbooks.end(); it++) {
                        if (*it == t) {
